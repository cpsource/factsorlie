...
def main():
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT idx, row_src, question_gz, created_at FROM querys ORDER BY idx DESC LIMIT %s",
                (args.limit,),
            )
            rows = cur.fetchall()
    conn.close()
    ...
    print(f"{'idx':<6} {'src':<8} {'created_at':<28} {'question'}")
    print("-" * 80)
    for idx, row_src, question_gz, created_at in rows:
        question = gzip.decompress(bytes(question_gz)).decode("utf-8")
        # Truncate long questions for display
        if len(question) > 60:
            question = question[:57] + "..."
        print(f"{idx:<6} {row_src:<8} {str(created_at):<28} {question}")
